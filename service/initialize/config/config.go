...
func getDefaultConfig() map[string]map[string]string {
	return map[string]map[string]string{
		"base": {
			"http_port":        "9090",
			"source_path":      "/data/uploads",      // 存放文件的路径
			"source_temp_path": "/data/uploads/temp", // 存放文件的缓存路径
		},
		"sqlite": {
			"file_path": "/data/database/database.db",
		},
	}
...
func ConfigInit() (*iniConfig.IniConfig, error) {
	...
	if config, err, errCode := Conf(getDefaultConfig()); err != nil && errCode == 0 {
		...
		global.Logger.Infoln("配置文件已经自动生成'/data/conf/conf.ini',将再次读取配置")
		// 创建成功再次读取文件
		if configAgain, errAgain, _ := Conf(getDefaultConfig()); errAgain != nil {
			return nil, errAgain
		} else {
			global.Logger.Infoln("尝试读取配置文件'/data/conf/conf.ini',二次读取配置文件成功")
...
func Conf(defaultConfig map[string]map[string]string) (config *iniConfig.IniConfig, err error, errCode int) {
	...
	exists, err := cmn.PathExists("/data/conf/conf.ini")
	if exists {
		config = iniConfig.NewIniConfig("/data/conf/conf.ini") // 读取配置
...
func CreateConfExample(confName string, targetName string) (err error) {
	...
	exists, err := cmn.PathExists("/data/conf/" + targetName)
	if err != nil {
		return
	}
	if !exists {
		if err = cmn.AssetsTakeFileToPath(confName, "/data/conf/"+targetName); err != nil {
			return
		}
	}
